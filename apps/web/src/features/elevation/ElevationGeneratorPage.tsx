...
export default function ElevationGeneratorPage() {
  ...
  const makeParams = (
    ...
    return {
      ...
      plyInward,
      minPieceWidth,
      // 조각 배치는 두 모드 모두 시공성 우선(창 좌우 일직선 절단·재분할·버림).
      // 물량 최소는 2P 시작 위치를 고르는 기준만 다르다(computePlyDev).
      placement: "constructability",
      discardWidth,
      constructMinPieceWidth: constructMinW,
      openings: opsDevX,
    };
  ...
  const computePlyDev = (
    ...
    // 물량 최소도 1P 는 시공성 우선 그대로(SP 최적배치 미적용)
    const dev1 =
      !minWaste && optimizeSP ? developPlyMinBoards(p1).dev : developPly(p1);
    ...
    //  남원주역세권1차 84C 2P: 경고 우선 48판 ↔ 판수 우선 42판, 현장 도면 44판)
  ...
  const handleExportSiteReportCsv = () => {
    ...
    rows.push(
      `동,단위세대 타입,1~3F,지붕층,기준층,세대수,층고(1~3F/지붕/기준),${arCols},${cmpCols}`
    );
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        style={{
                          backgroundColor: KIND_COLOR[pr.kind] + "44",
                          borderColor: KIND_COLOR[pr.kind],
                        }}
                      />
                      {pr.label}
                    </button>
                ...
                <div className={stacked ? "grid grid-cols-3 gap-1.5" : "flex items-end gap-1"}>
                  {(
                    [
                      ...
                      ["높이", preset.height, (v: number) => updatePreset(preset.id, { height: v })],
  ...
  const layerControlNode = (
            <div className="flex flex-col gap-1 pt-0.5">
              ...
              <select
                value=""
                onChange={ev => {
                  if (ev.target.value) toggleLayer(ev.target.value);
                  ev.target.value = "";
                }}
                disabled={!parsed}
                className="h-6 w-[124px] rounded border border-slate-300 bg-white px-1.5 text-[11px] text-slate-700 disabled:opacity-40"
              >
                <option value="">
                  도면층 {parsed?.layers.length ?? 0}개 · 켜기/끄기
                </option>
                {parsed?.layers.map(ly => (
                  <option key={ly} value={ly}>
                    {hiddenLayers.has(ly) ? "☐" : "☑"} {ly}
                  </option>
                ))}
              </select>
            </div>
    ...
    <>
      ...
      >
        ...
        <CadRibbon
          ...
          onAutoFullscreen={setAutoFullscreen}
              ...
              {activeProjectData.revisions.map(r => (
                ...
                >
                  ...
                  <button
                    ...
                    className="shrink-0 rounded border border-[#004791] px-2 py-0.5 font-semibold text-[#004791] hover:bg-blue-50"
                  >
                    불러오기
                  </button>
                  <button
                    ...
                    className="shrink-0 rounded px-1.5 py-0.5 text-rose-500 hover:bg-rose-50"
                    title="REV 삭제"
                  >
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-2 pt-1">
                  ...
                  {placement === "min-waste" && (
                    <p className="text-[10.5px] leading-snug text-slate-500">
                      물량 최소: 조각 배치(1P·2P)는 시공성 우선과 같고, 2P 시작 위치를
                      벽 면마다 판수가 가장 적게 드는 쪽으로 고릅니다.
                    </p>
                  )}
                    <>
                      <LabelInput
        ...
        <CadStatusBar
          ...
          hint={
            mode === "trace"
              ...
              : mode === "place"
                ? "프리셋 배치 — 선택한 오프닝을 벽 위에 클릭해 배치"
                : mode === "two-point"
                  ? "정밀 배치 — 시작점과 끝점 두 곳을 클릭"
                  : mode === "auto"
                    ? "창호 자동 — 평면의 창호 라벨(예: 18×11.8)을 클릭"
                    : mode === "measure-dist"
                      ? "거리 측정 — 두 점 클릭 (Esc 초기화)"
                      : mode === "measure-path"
                        ? "연속 측정 — 점을 이어 클릭, 누적 길이 표시 (Esc 초기화)"
                        : mode === "measure-area"
                          ? "면적 측정 — 꼭짓점 3개 이상 클릭 (Esc 초기화)"
                          : mode === "edit"
                            ? "편집 — 요소 클릭 후 드래그 이동 · Del 삭제 · Ctrl+Z 되돌리기"
                            : parsed
                              ? "명령 대기 중 — 리본에서 명령을 선택하세요 (휠: 확대·축소, 드래그: 화면 이동)"
                              : "DXF 파일을 업로드하세요 — 리본 [삽입] 탭 > DXF 업로드"
          }
        />
