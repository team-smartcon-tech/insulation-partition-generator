...
- [x] 8. UI 연동 — 엔진 HTTP API + 오토콘 리본 [적산] 탭

**2부 · 기성**
- [x] 9. 세대 대장 (Excel 붙여넣기 · 규칙 생성기 · 필로티/결번 제외)
- [x] 10. 범위 문자열 파서 + 미리보기
- [x] 11. 진도 모델(누계) + 차수 관리 + 스냅샷
- [x] 12. 그리드 UI (층×호 클릭·드래그 + 범위 문자열 역변환)
- [x] 13. 기성 산출 + 검증 규칙
...
```bash
# 1) 엔진 API (별도 터미널)
cd engine && python -m finish_takeoff.server        # 127.0.0.1:8901

# 2) 오토콘 웹
pnpm dev                                            # localhost:5173
#    → 세대 단열재 나누기도 → 리본 [적산] 탭 → 물량 산출 / 세대 대장 / 기성 관리
```

배포 시 엔진 주소는 `VITE_TAKEOFF_API` 로 지정한다.
